const contactTitle = `
I am interested in working with any company that thinks my skill will be helpful for them. 
If you are looking for someone like me, please let me know. Or you can just 'say hi' to me.`;

const contactWays = [
{ title: "Email", name: "towhidmorol46@gmail.com" },
{ title: "Phone", name: "++0 1928182891" },
{ title: "Skype", name: "towhidmorol46@gmail.com" },
{ title: "Address", name: "Gopalganj, Dhaka, Bangladesh" },
];

export const contactData = {contactTitle, contactWays}